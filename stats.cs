...
List<int> pontostats = Valores.pontostats; List<string> stats = Valores.stats; int mudança = Valores.mudança; string MB = Valores.MudançaBruta;
        ...
        Console.WriteLine("(Para acresentar seus status digite (Abreviação do status) ou Encerrar) (APENAS UM NÚMERO INTEIRO)"); //EU CONSERTEI O BUG DA LEPTOSPIROSE
switch(Console.ReadLine().ToUpper()){case $"FOR":
